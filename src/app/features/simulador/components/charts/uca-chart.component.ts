...
import { UCAResult } from '../../models/reologia.model';

Chart.register(...registerables);

@Component({
  selector: 'app-uca-chart',
  standalone: true,
  template: `
...
export class UcaChartComponent implements AfterViewInit, OnChanges {
  @Input() data: UCAResult | null = null;
  ...
  save(): void {
    ...
    a.href = url;
    a.download = 'uca-resistencia.png';
    a.click();
  }
  ...
  private update(): void {
    ...
    const d = this.data;
    this.chart.data.labels = d.t.map(v => v.toFixed(1));
    (this.chart.data.datasets[0] as any).data = d.strength;
    (this.chart.data.datasets[1] as any).data = d.transit;
    this.chart.update();
  }
  ...
  private config(): ChartConfiguration {
    ...
    return {
      type: 'line',
      data: {
        labels: d ? d.t.map(v => v.toFixed(1)) : [],
        datasets: [
          { label: 'Resistência (psi)', data: d?.strength ?? [], borderColor: '#8b5cf6', backgroundColor: 'rgba(139,92,246,0.08)', yAxisID: 'y', tension: 0.3, pointRadius: 0 },
          { label: 'Transit Time (µs/ft)', data: d?.transit ?? [], borderColor: '#f59e0b', backgroundColor: 'transparent', yAxisID: 'y1', tension: 0.3, pointRadius: 0, borderDash: [4,2] },
        ],
      },
      options: {
        ...
        scales: {
          ...
          y: { title: { display: true, text: 'Resistência (psi)' }, min: 0 },
          y1: { position: 'right', title: { display: true, text: 'µs/ft' }, grid: { drawOnChartArea: false } },
        },
      },
    };
  }
